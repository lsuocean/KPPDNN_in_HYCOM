...
!
      ...
      real, save, dimension(1-nbdy:idm+nbdy,1-nbdy:jdm+nbdy) ::  &
       ...
       hbl_old           ! old hbl from previous time step
...
! --- kpp variables
...
#if defined KPP_DNN
      ,zgrid_dnn       & !  grid levels in meters (for DNN input prep)
#endif
      ...
      ,vtc1            & ! vtc value only used when unstable in LF17
...
!
      integer, save :: &
                     ...
                     empbal,sssbal, &
                     difsmo,disp_count,dnnflg,hsblmt
!
      real,    parameter :: &
...
#if defined KPP_DNN
! --- (Yuan 11/20/2024 added) DNN related variables
       character(len=100)       ::      f_eta
       character(len=100)       ::      f_eta0
       character(len=100)       ::      f_epsl
       character(len=100)       ::      f_mean
       character(len=100)       ::      f_sd
       character(len=100)       ::      f_wm
       character(len=100)       ::      f_ws

       ! Normalizing parameters
       real, dimension(:), allocatable   ::   input_mean
       real, dimension(:), allocatable   ::   input_sd

       ! DNN inputs
       integer                  ::      n_inp
       integer                  ::      npfl
       !real(rk)                 ::      dnn_input(73) !73 !106
       real, dimension(:), allocatable   ::   dnn_input
       real, dimension(:), allocatable   ::   dnn_input1
       real, dimension(:), allocatable   ::   z_inp
       real, dimension(:), allocatable   ::   t_tmp
       real, dimension(:), allocatable   ::   s_tmp
       real, dimension(:), allocatable   ::   th_tmp
       real, dimension(:), allocatable   ::   wm_tmp
       real, dimension(:), allocatable   ::   ws_tmp 
# if defined STOKES
       real, dimension(:), allocatable   ::   us_tmp
       real, dimension(:), allocatable   ::   vs_tmp
# endif
       real                     ::      tmp_fac(1)
       real                     ::      tmp_tau
       real                     ::      tmp_sp
       real                     ::      tmp_u,tmp_v,tmp_ht,tmp_sw
       real                     ::      tmp_ep,tmp_lt,tmp_dp,tmp_hl
       real                     ::      tmp_var,tmp_in
       real,public              ::      Efactor
       real,public              ::      Vtfac, Vtfac0
       real,public              ::      vt_limit,ef_limit,vt_low

       ! FKB model variables
       type(network_type)                    ::   model_eta
       type(network_type)                    ::   model_eta0
       type(network_type)                    ::   model_epsl
       type(network_type)                    ::   model_wm
...
!
      ...
      allocate( &
           ...
           dispqd_mn(1-nbdy:idm+nbdy,1-nbdy:jdm+nbdy), &
             hbl_old(1-nbdy:idm+nbdy,1-nbdy:jdm+nbdy) )
...
!
      ...
      call mem_stat_add( (idm+2*nbdy)*(jdm+2*nbdy) ) !real=2*int
       ...
       if (dnnflg.eq.1) then
         n_inp = npfl+9
         allocate(input_mean(1:10))
         input_mean = 0.0
         allocate(input_sd(1:10))
         input_sd = 0.0
       endif
       if (dnnflg.eq.2) then
         n_inp = npfl*3+9
         allocate(input_mean(1:12))
         input_mean = 0.0
         allocate(input_sd(1:12))
         input_sd = 0.0
       endif
       allocate(dnn_input(1:n_inp))
       allocate(dnn_input1(1:(n_inp-1)))
       allocate(z_inp(1:npfl))
       allocate(s_tmp(1:npfl))
       allocate(t_tmp(1:npfl))
       allocate(th_tmp(1:npfl))
       allocate(wm_tmp(1:npfl))
       allocate(ws_tmp(1:npfl))
# if defined STOKES
       allocate(us_tmp(1:npfl))
       allocate(vs_tmp(1:npfl))
# endif
       dnn_input = 0.0
       dnn_input1 = 0.0
       z_inp = 0.0
...
#if defined KPP_DNN
                zgrid_dnn(1-nbdy:idm+nbdy,1-nbdy:jdm+nbdy,kdm+1), &
#endif
