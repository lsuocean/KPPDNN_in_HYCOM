...
! local variables for kpp mixing
      ...
      real zscale              ! thickness from ocean surface
      real C_ek                ! Coefficient in Cr equation
      real Kern                ! J(z) term in Cr equation
      real fcor                ! Coriolis coefficient
      real shear2              ! shear term in Cr equation
      real stabil              ! stability term in Cr equation
      real mean_wm,sd_wm,mean_ws,sd_ws ! parameter for wm ws dnns
!
...
#ifdef KPP_DNN
      real                 :: t_tmp1(kdm+1),s_tmp1(kdm+1)
      real                 :: th_nn(kdm+1), z_tmp(0:(kdm+1))
      real                 :: us_in(0:(kdm+1)),vs_in(0:(kdm+1))
      real                 :: z_min
      integer              :: i_tmp
# if defined STOKES
!      real                 :: ws0, ws1
...
! --- local 1-d arrays for iteration loops
      real uold(kdm+1),vold (kdm+1),told (kdm+1), &
           sold(kdm+1),thold(kdm+1),FC(kdm+1),Cr(kdm+1),vt0(kdm+1)
!
...
!
       if (abs(plon(i,j)+79.1999).le.0.001.and.  &
                 abs(plat(i,j)-23.4743).le.0.001) then
       endif
...
! --- calculate layer thicknesses in m
      ...
      enddo
!
!     calculate layer thicknesses in m for DNNs
       do k=1,(kk)
        if (k.eq.1) then
          hwide(k)=dp(i,j,k,n)*qonem
          zgrid_dnn(i,j,k)=-.5*hwide(k)
        else if (k.lt.kk) then
          hwide(k)=dp(i,j,k,n)*qonem
          zgrid_dnn(i,j,k)=zgrid_dnn(i,j,k-1)-.5*(hwide(k-1)+hwide(k))
        else if (k.eq.(kk)) then
          hwide(k)=dp(i,j,k,n)*qonem
          zgrid_dnn(i,j,k)=zgrid_dnn(i,j,k-1)-.5*(hwide(k-1)+hwide(k))
          zgrid_dnn(i,j,k+1)=zgrid_dnn(i,j,k)-.5*hwide(k)
        endif
      enddo


      do k = 1,kdm+1
        zgrid_dnn(i,j,k) = pij(k)*qonem*oneta(i,j,n)
      enddo
...
! --- calculate interface variables required to estimate interior diffusivities
        ...
        enddo
!
! --- (Yuan 04/29/2025) prepare for Cr hsbl finder
        if (hsblmt.eq.2) then
         FC = 0.0
         C_ek = 258.0
         do k=2,klist(i,j)
           zscale = zgrid(i,j,1) - zgrid(i,j,k)
           Kern = zscale/(zscale + 0.1*hbl_old(i,j))
           fcor = 2.0*7.2921e-5*sin(plat(i,j)*3.14159265358979/180.0)
           shear2=2.0*((uold(k-1)-uold(k))**2+(vold(k-1)-vold(k))**2) &
                / (hwide(k)+hwide(k-1))
           stabil = 0.5*(hwide(k)+hwide(k-1)) * &
                (dbloc(k)/ricr + C_ek*fcor**2)
           FC(k) = FC(k-1) + Kern * (shear2 - stabil)
           if (abs(FC(k)).ge.0.5) then
             !write(*,*) 'ck0',zscale,Kern,fcor,shear2,stabil,FC(k)
           endif
         enddo
        endif
!       
...
# if defined KPP_DNN
        ...
        if (dnnflg.gt.0) then
          z_tmp(0) = 0.0
          z_tmp(1:(kdm+1)) = zgrid(i,j,:)
          t_tmp1 = told
          s_tmp1 = sold

          ! Calculate N-square used as DNN inputs
          do k=1,klist(i,j)
            if     (max(dbloc(k),dbloc(k+1)).gt.0.0) then
              ...
              if     (bfq.gt.0.0) then
                ...
                th_nn(k)=0.0  !neutral or unstable
              endif
            else
              th_nn(k)=0.0  !neutral or unstable
            endif
            th_nn(k) = th_nn(k) * th_nn(k)
          enddo

          th_nn(1) = th_nn(2)

          k = klist(i,j)
          z_min = max(-296.6, z_tmp(klist(i,j)))

          do i_tmp=1,(npfl)
            z_inp(i_tmp) = hbl_old(i,j)/32.0*(i_tmp-1)
          enddo

          t_tmp = interp_linear(-z_tmp(1:k),t_tmp1(1:k),z_inp)
          s_tmp = interp_linear(-z_tmp(1:k),s_tmp1(1:k),z_inp)
          th_tmp = interp_linear(-z_tmp(1:k),th_nn(1:k),z_inp)
# if defined STOKES
          us_in(0) = usds(i,j)
          us_in(1:k) = usdp(i,j,1:k)
          vs_in(0) = vsds(i,j)
          vs_in(1:k) = vsdp(i,j,1:k)
          us_tmp = interp_linear(-z_tmp(1:k),us_in(1:k),z_inp)
          vs_tmp = interp_linear(-z_tmp(1:k),vs_in(1:k),z_inp)

# endif

          ! prepare for DNN input array
          tmp_tau = sqrt(surtx(i,j)**2.0+surty(i,j)**2.0)
          tmp_sp = sqrt(tmp_tau/1.025/0.0013 )
          tmp_u = tmp_sp*surtx(i,j)/tmp_tau
          tmp_v = tmp_sp*surty(i,j)/tmp_tau

          ! To ensure the inputs are within limits seen during DNN
          ! training
          tmp_u = max(tmp_u, -15.68989)
          tmp_u = min(tmp_u, 17.96371)

          tmp_v = max(-21.01194,tmp_v)
          tmp_v = min(21.30692, tmp_v)

          tmp_ht = max(-1315.60832,sflux1)
          tmp_ht = min(83.39904,tmp_ht)

          tmp_sw = min(994.5496,sswflx(i,j))
          tmp_sw = max(0.0, tmp_sw)

          tmp_ep = min(3.673463e-07,-wtrflx(i,j)/rhoref)
          tmp_ep = max(-4.945771e-06,tmp_ep)

          tmp_lt = plat(i,j)

          tmp_dp = max(-300.0,z_tmp(k))
          tmp_dp = min(-2.0, tmp_dp)

          tmp_hl = min(-1.0, -hbl_old(i,j))
          tmp_hl = max(-201.0, tmp_hl)

          if (dnnflg.eq.1) then
            dnn_input(1:npfl) = (th_tmp - input_mean(1))/input_sd(1)
            call fnm(tmp_u,input_mean(2),input_sd(2), tmp_var)
            dnn_input(1*npfl + 1) = tmp_var

            call fnm(tmp_v,input_mean(3),input_sd(3), tmp_var)
            dnn_input(1*npfl + 2) = tmp_var

            call fnm(tmp_ht,input_mean(4),input_sd(4), tmp_var)
            dnn_input(1*npfl + 3) = tmp_var

            call fnm(tmp_sw,input_mean(5),input_sd(5), tmp_var)
            dnn_input(1*npfl + 4) = tmp_var

            call fnm(tmp_ep,input_mean(6),input_sd(6), tmp_var)
            dnn_input(1*npfl + 5) = tmp_var

            call fnm(tmp_lt,input_mean(7),input_sd(7), tmp_var)
            dnn_input(1*npfl + 6) = tmp_var

            call fnm(tmp_dp,input_mean(8),input_sd(8), tmp_var)
            dnn_input(1*npfl + 7) = tmp_var

            call fnm(tmp_hl,input_mean(9),input_sd(9), tmp_var)
            dnn_input(1*npfl + 8) = tmp_var
         endif
...
# if defined STOKES
          if (dnnflg.eq.2) then
            dnn_input(1:npfl) = (th_tmp - input_mean(1))/input_sd(1)
       dnn_input((1*npfl+1):(2*npfl))=(us_tmp-input_mean(2))/input_sd(2)
       dnn_input((2*npfl+1):(3*npfl))=(vs_tmp-input_mean(3))/input_sd(3)
            call fnm(tmp_u,input_mean(4),input_sd(4), tmp_var)
            dnn_input(3*npfl + 1) = tmp_var

            call fnm(tmp_v,input_mean(5),input_sd(5), tmp_var)
            dnn_input(3*npfl + 2) = tmp_var

            call fnm(tmp_ht,input_mean(6),input_sd(6), tmp_var)
            dnn_input(3*npfl + 3) = tmp_var

            call fnm(tmp_sw,input_mean(7),input_sd(7), tmp_var)
            dnn_input(3*npfl + 4) = tmp_var

            call fnm(tmp_ep,input_mean(8),input_sd(8), tmp_var)
            dnn_input(3*npfl + 5) = tmp_var

            call fnm(tmp_lt,input_mean(9),input_sd(9), tmp_var)
            dnn_input(3*npfl + 6) = tmp_var

            call fnm(tmp_dp,input_mean(10),input_sd(10), tmp_var)
            dnn_input(3*npfl + 7) = tmp_var

            call fnm(tmp_hl,input_mean(11),input_sd(11), tmp_var)
            dnn_input(3*npfl + 8) = tmp_var
...
# endif
          ...
          if (dnnflg.ge.1) then
            dnn_input1 = dnn_input(1:(n_inp-1))
            tmp_fac = model_eta0 % output(dnn_input1)
            if (tmp_fac(1).lt.0.5) then
               Vtfac = 0.0
            else
               tmp_fac = model_eta % output(dnn_input1)
               Vtfac = 10.0**tmp_fac(1)
               Vtfac = max(0.0, Vtfac)
               Vtfac = min(vt_limit, Vtfac)
            endif
          endif
        endif
...
! --- diagnose hbl and nbl
        if (hsblmt.eq.1) then ! Bulk Richardson number method
          do k=2,nbl
            case=-zgrid(i,j,k)
...
! --- hbl = case = -zgrid(i,j,k) #using a lookup table, not direct calc.
...
! --- compute bulk richardson number at new level
            rib(kdn)=ritop(k)/(dvsq(k)+max(Vtfac,vt_low)*vtsq+epsil)
...
! --- compute the turbulent shear contribution to rib
            ...
            vt0(k) = vtsq
            Cr(k) = FC(k) + vtsq
            if (nbl.eq.klist(i,j).and.Cr(k).le.0.0) then
! ---       interpolate to find hbl as the depth where Cr<=0
              if     (k.eq.2 .or. hblflg.eq.0) then  !nearest interface
                hbl = -zgrid(i,j,k-1)+0.5*hwide(k-1)
              elseif (k.lt.4 .or. hblflg.eq.1) then  !linear
                hbl = -zgrid(i,j,k-1)+ &
                       (zgrid(i,j,k-1)-zgrid(i,j,k))* &
                       (0-Cr(k-1))/(Cr(k)-Cr(k-1)+epsil)
              else !quadratic
!
! ---         Determine the coefficients A,B,C of the polynomial
! ---           Y(X) = A * (X-X2)**2 + B * (X-X2) + C
...
# if defined KPP_DNN
        if (dnnflg.ge.1) then
          ...
          if (dnnflg.eq.2) then
            ...
            vs_in(0) = vsds(i,j)
            vs_in(1:k) = vsdp(i,j,1:k)
            us_tmp = interp_linear(-z_tmp(0:k),us_in(0:k),z_inp)
            vs_tmp = interp_linear(-z_tmp(0:k),vs_in(0:k),z_inp)
          endif
          tmp_hl = min(-1.0, -hbl_old(i,j))
          tmp_hl = max(-201.0, tmp_hl)
          ...
          if (dnnflg.eq.1) then
            ...
            call fnm(tmp_hl,input_mean(9),input_sd(9), tmp_var)
            dnn_input(1*npfl+8) = tmp_var
            call fnm(Vtfac,input_mean(10),input_sd(10), tmp_var)
            dnn_input(1*npfl+9) = tmp_var
          endif
          if (dnnflg.eq.2) then
            dnn_input(1:npfl) = (th_tmp - input_mean(1))/input_sd(1)
       dnn_input((1*npfl+1):(2*npfl))=(us_tmp-input_mean(2))/input_sd(2)
       dnn_input((2*npfl+1):(3*npfl))=(vs_tmp-input_mean(3))/input_sd(3)
            call fnm(tmp_hl,input_mean(11),input_sd(11), tmp_var)
            dnn_input(3*npfl+8) = tmp_var
            call fnm(Vtfac,input_mean(12),input_sd(12), tmp_var)
            dnn_input(3*npfl+9) = tmp_var
          endif
        endif
          
        if (dnnflg.ge.1) then
          tmp_fac = model_epsl % output(dnn_input)
          EFactor = 10.0**tmp_fac(1)
          if (EFactor.ne.EFactor) then
            EFactor = 1.0
          endif
          EFactor = max(0.3, EFactor)
...
! --- compute boundary layer diffusivities at the interfaces
            blmc(k,1)=EFactor*hbl*wm*sigg*(1.+sigg*gm)
            blmc(k,2)=EFactor*hbl*ws*sigg*(1.+sigg*gs)
            blmc(k,3)=EFactor*hbl*ws*sigg*(1.+sigg*gt)
!
...
!
        ...
        gs=aa1+aa2*gat1(2)+aa3*dat1(2)
        gt=aa1+aa2*gat1(3)+aa3*dat1(3)
        dkm1(1)=EFactor*hbl*wm*sigg*(1.+sigg*gm)
        dkm1(2)=EFactor*hbl*ws*sigg*(1.+sigg*gs)
        dkm1(3)=EFactor*hbl*ws*sigg*(1 +sigg*gt)
!
...
!
          rib(kup)=ritop(k)/(dvsq(k)+vtsq*Vtfac+epsil)
...
!
      subroutine fnm(tmp_in, mean_in, sd_in, sigmoid_value)
        implicit none
        real, intent(in) :: tmp_in      ! Input temperature
        real, intent(in) :: mean_in     ! Mean value
        real, intent(in) :: sd_in       ! Standard deviation
        real, intent(out) :: sigmoid_value  ! Result of sigmoid function
        real :: tmp_val                ! Temporary variable

        ! Calculate tmp_val
        tmp_val = (tmp_in - mean_in) / sd_in

        ! Compute the sigmoid function
        sigmoid_value = 1.0 / (1.0 + exp(-tmp_val))
      end subroutine fnm
